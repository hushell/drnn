function [accs mprs] = test_TC_xvalid_140(data_train, data_eval, save_dir, use_gt)
...
for xv = 1:5
  ...
  elseif use_gt
      ...
      error('LR model has not been learned!\n');
  end

  allData = allDataFold{xv};

  % compute all parse trees
  tree_file = ['../../output/iccv09-allTrees-140-fold' num2str(xv) '.mat'];
  if exist(tree_file,'file')
      ...
      for i = 1:length(allData)
          if xv == 5
              ucm_dir = 'iccv09data/ucm2/iccv09_';
          else
              ucm_dir = 'iccv09data_train/ucm2/iccv09_train_';
              i = (xv-1)*143 + i;
          end
          fprintf('computing parse tree %d...\n', i);
          load(['../../../BSR/' ucm_dir num2str(i) '.mat']);
  ...
  p_connect_file = ['p_connect_star_140' 'fold_' num2str(xv) '.mat'];
  ...
  for i = 1:143
    ...
    if use_gt %NOTE: change lik_type in tree_cut_new to 1
      %[Q,cuts,labs,forest] = tree_cut_func(allData{i}, allTrees{i}, theta_plus, nclass, p_connect_star(i));
