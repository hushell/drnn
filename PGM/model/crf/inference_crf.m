function [mu, flag] = inference_crf(w_crf, X, nlabel, nMF)
...
% node potential
mu_node = w_crf.nodeWeights'*X.nodeFeatures;
...
for ei = 1:X.numNodes,
    edgeFeat(:,:,ei,xj(xi == ei)) = reshape(sum(bsxfun(@times,X.edgeFeaturesrs(:,:,:,xi == ei),w_crf.edgeWeights),1),nlabel,nlabel,1,sum(xi == ei));
    edgeFeat(:,:,ei,xi(xj == ei)) = reshape(sum(bsxfun(@times,X.edgeFeaturesrs(:,:,:,xj == ei),w_crf.edgeWeights),1),nlabel,nlabel,1,sum(xj == ei));
end
