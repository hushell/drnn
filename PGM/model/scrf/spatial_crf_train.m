% spatial conditional random fields on LFW part label dataset.
% 
...
%   skip_edgeonlyup : skip edge pretrain if true
% 
% output
...
function w_scrf = spatial_crf_train(w_scrf, Y, X, dim, nlabel, l2reg_node, l2reg_edge, skip_edgeonlyup)

if ~exist('skip_edgeonlyup','var'),
    skip_edgeonlyup = 0;
end
...
% jointly optimize
fprintf('SCRF: joint node and edge optimization\n');
theta = [w_scrf.nodeWeights(:) ; w_scrf.edgeWeights(:)];
opttheta = minFunc(@(p) cost_spatial_crf_mf(p, w_scrf, X, Y, gn_pos, ge_pos, l2reg_node, l2reg_edge, dim, nlabel, dimNodeFeat, dimEdgeFeat), theta, options);
clear w_scrf;
w_scrf = unroll_pars_scrf(opttheta, dim, nlabel, dimNodeFeat, dimEdgeFeat);
