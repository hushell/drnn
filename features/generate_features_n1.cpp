...
{
  /**
   * | color [0,63] | texture [64,127] | position [128,139] | context_color [140,203] | context_text [204,267] |
   */
  ...
  int ctColorFOffset = posFOffset + numPosF;
  int ctTexFOffset = ctColorFOffset + numClusters;
  double uw = (int)(ww) / 3.0;
  double uh = (int)(hh) / 4.0;
  
  //int numFeatures = numClusters + numGridFeatures;
  int numFeatures = numClusters + numTextClusters + numPosF + numClusters + numTextClusters;
	  ...
	  if (neiMode > 0)
	  {
	    //++numInSPNeigh[sp];
	  	for (int ni = 0; ni < numSP; ni++)
	  	{
	  	    ...
	  	    {
	  			  ++numInSPNeigh[ni];
	  			  //++instanceNF[ni][ctTexFOffset + texcluster];
  ...
  vector<int> numInSPCompare(numInSP.size(), 0);
  for (int ni = 0; ni < numSP; ni++)
  {
  ...
  // DEBUG code end
	...
	for(int i=0; i<numSP; i++)
	{
		...
		if (neiMode > 0)
		{
			for (int ni = 0; ni < numSP; ni++)
			{
				if (neigh[i][ni] > 0)
				{
					...
					{
						instanceNF[i][ctColorFOffset + ci] += instanceNF[ni][ci];
					}

					for (int ti = 0; ti < numTextClusters; ti++)
					{
						instanceNF[i][ctTexFOffset + ti] += instanceNF[ni][texFOffset + ti];
					}
				}
			}
		}
		...
		// normalize pos part
		for(int j=posFOffset; j<ctColorFOffset; j++)
		{
			instanceNF[i][j] /= numInSP[i];
			(*outfile) << instanceNF[i][j] << '\t';
		}
		...
		if (neiMode > 0)
		{
			for(int j=ctColorFOffset; j<ctTexFOffset; j++)
			{
				instanceNF[i][j] /= numInSPNeigh[i];
				(*outfile) << instanceNF[i][j] << '\t';
			}

			for(int j=ctTexFOffset; j<numFeatures; j++)
			{
				instanceNF[i][j] /= numInSPNeigh[i];
				(*outfile) << instanceNF[i][j] << '\t';
			}
